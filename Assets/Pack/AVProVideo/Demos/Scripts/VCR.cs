...
namespace RenderHeads.Media.AVProVideo.Demos
{
	...
	public class VCR : MonoBehaviour 
	{
		...
		public List<string> _videoFiles;
...
//				SetButtonEnabled( "PlayButton", true );
		...
		public void OnVideoSeekSlider()
		{
            ...
            {
				pressedVal = _videoSeekSlider.value;
			}

			if (PlayingPlayer && _videoSeekSlider && _videoSeekSlider.value != _setVideoSeekSliderValue && _videoSeekSlider.value >= pressedVal)
			{
				PlayingPlayer.Control.Seek(_videoSeekSlider.value * PlayingPlayer.Info.GetDurationMs());

				Debug.Log(GetReadableTimeByMs(PlayingPlayer.Control.GetCurrentTimeMs()));
			}
		}
		public string GetReadableTimeByMs(float ms)
		{
			TimeSpan t = TimeSpan.FromMilliseconds(ms);
			return $"0{t.Minutes}:{t.Seconds}:{t.Milliseconds}";
		}
		public float ConvertTimeByMs(string readableTime)
		{
			TimeSpan ts = new TimeSpan(
				0,
				int.Parse(readableTime.Split(':')[0]),
				int.Parse(readableTime.Split(':')[1]),
				int.Parse(readableTime.Split(':')[2]),
				int.Parse(readableTime.Split(':')[3])
				);
